		...
		if(first) {
			...
			g.legendItemContainer = g.chartElement().append("g")
				...
				columnLabels.transition()
					.attr("x", function(d,i) {
						return x = g.xAxis().scale(g.xAxisRef()[0].data[i])  - columnWidth/2
						}
					.attr("y",function(d,i) {return (g.yAxis()[yAxisIndex].scale(d)-g.yAxis()[yAxisIndex].scale(Gneiss.helper.columnXandHeight(d,g.yAxis()[yAxisIndex].scale.domain()))) >= 0 ? g.yAxis()[yAxisIndex].scale(Gneiss.helper.columnXandHeight(d,g.yAxis()[yAxisIndex].scale.domain())) : g.yAxis()[yAxisIndex].scale(d)})
